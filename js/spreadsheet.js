...
//para guardar los anchos de las columnas
var colWidth = new Array();

var fnSaveColWidth = function(){
...
function display() {
  ...
  if (isWriteable) {
    ...
    if (saveMethod!="") out += "<a href='#' onclick='eval(saveMethod); return false;' accesskey='s'>"+trans("Save")+"</a> - ";
  }
  ...
  if (allowPaging) {
    ...
    if (col0-cols>=0) {
      out += "<a href='#' onclick='col0 -= cols; currCol -= cols; display(); return false;'>"+trans("&lt;&lt;")+"</a> - ";
    } else out += "&lt;&lt; - ";
    out += "<a href='#' onclick='col0 += cols; currCol += cols; display(); return false;'>"+trans("&gt;&gt;")+"</a> - ";
    if (row0-rows>=0) {
      out += "<a href='#' onclick='row0 -= rows; currRow -= rows; display(); return false;'>"+trans("Up")+"</a> - ";
    } else out += trans("Up")+" - ";
    out += "<a href='#' onclick='row0 += rows; currRow += rows; display(); return false;'>"+trans("Down")+"</a> -&nbsp;";
    out += "</td><td nowrap>";
    out += "<input type='text' value='"+cols+"' style='width:28px; text-align:center;' id='cols' onfocus='active=\"dimensions\";' onblur='active=\"content\";'> x ";
  ...
  out += "</td><td nowrap>";
  if (isWriteable) {
    out += "&nbsp; <input type='button' value='X' id='cancel' onclick='cancelCell();' disabled />&nbsp;";
    out += "<input type='button' value='Ok' id='save' onclick='saveCell();' disabled>&nbsp;";
  } else out += "&nbsp; ";
  if (view=="values") {
    ...
    //out += "<a href='#' onclick='view=\"formulas\"; display(); return false;'>"+trans("Values")+"</a> - ";
  } else if (view=="formulas") {
    //out += "<a href='#' onclick='view=\"styles\"; display(); return false;'>"+trans("Formulas")+"</a> - ";
  } else {
    //out += "<a href='#' onclick='view=\"values\"; display(); return false;'>"+trans("Styles")+"</a> - ";
  }
  ...
  //out += "<a href='#' onclick='view=\"values\"; display(); return false;' accesskey='1'></a>";
  //out += "<a href='#' onclick='view=\"formulas\"; display(); return false;' accesskey='2'></a>";
  //out += "<a href='#' onclick='view=\"styles\"; display(); return false;' accesskey='3'></a>";
  ...
  out += "<div id='toolbar' "+style+" class='toolbar'>";
  out += "<table cellpadding='0' cellspacing='0'><tr><td>";
  out += "<a href='#' onclick='if (confirm(\""+trans("Really close without saving changes ?")+"\")) load(init_data); return false;' accesskey='n'><div class='new'></div></a>&nbsp;";
  out += "<a href='#' onclick='loadCode(); return false;' accesskey='l'><div class='open'></div></a>&nbsp;";
  out += "<a href='#' onclick='eval(saveMethod); return false;' accesskey='s'><div class='save'></div></a>&nbsp;";
  out += "<a href='#' onclick='print(); return false;' accesskey='p'><div class='print'></div></a>&nbsp;";
  out += "</td><td>";
  out += "<a href='#' onclick='undoAction();'><div class='undo'></div></a>&nbsp;";
  out += "<a href='#' onclick='redoAction();'><div class='redo'></div></a>&nbsp;";
  out += "</td><td>";
  out += "<a href='#' onclick='cut();'><div class='cut'></div></a>&nbsp;";
  out += "<a href='#' onclick='cutcopy(\'copy\',\'#DDDDFF\'); return false;'><div class='copy'></div></a>&nbsp;";
  out += "<a href='#' onclick='paste();'><div class='paste'></div></a>&nbsp;";
  out += "</td><td>";
  out += "<a href='#' onclick='setBoldCellMatrix();'><div class='bold'></div></a>&nbsp;";
  out += "<a href='#' onclick='setItalicCellMatrix();'><div class='italic'></div></a>&nbsp;";
  out += "<a href='#' onclick='setUnderlineCellMatrix();'><div class='underline'></div></a>&nbsp;";
   out += "</td><td>";
  out += "<a href='#' onclick='colspan(); return false;'><div class='rowspan'></div></a>&nbsp;";
  out += "</td><td>";
 out += "</td><td>";
  out += "<a href='#' onclick='insertRow(); return false;'><div class='insert_row'></div></a>&nbsp;";
  out += "<a href='#' onclick='insertColumn(); return false;'><div class='insert_column'></div></a>&nbsp;";
  out += "</td><td>";
  out += "<a href='#' onclick='sortAsc();'><div class='sort_asc'></div></a>&nbsp;";
  out += "<a href='#' onclick='sortDesc();'><div class='sort_desc'></div></a>&nbsp;";
  out += "</td></tr>";
  out += "</table>";
  out += "</div>";
  ...
  out += "</tr>";
  */
  //modificando para que sea con thead
  /*
  ...
  */
  ...
  //////////////////////////////////////////////////////////////////////////////////////
  //////////////////////////////////////////////////////////////////////////////////////
  //agregando el evento para que las columnas sean sizeables
...
function print() {
  ...
  obj.document.write("<html><head><link media='all' href='css\\"+printstyle+"' rel='stylesheet' type='text/css'/></head>\n");
...
///////////////////////////////////////////
//                                       //
//  FUNCIONES AGREGADAS PARA EL TOOLBAR  //
//                                       //
///////////////////////////////////////////
...
function setBold() {
  var obj2 = resolveCell(currRow,currCol);
  var pos_bold = -1;
  var value = previewCell(cells[currRow][currCol][0],0);
  var style = cells[currRow][currCol][1];
  value = formatValue(value,style);
  style = htmlEscape(formatStyle(style,value),false);
  //alert(style);
  var arrstyle = style.split(";");
  for (var i=0; i<arrstyle.length; i++){
    if (arrstyle[i].replace(" ","") == "font-weight:bold"){
      pos_bold = i;
      break;
    }
  }
  ...
  cells[currRow][currCol][1] = style;
  //estilo anterior
  //si tiene bold => lo quita
  //si no tiene bold => lo aumenta
  ...
  value = htmlEscape(value,true);
  var val = "<div style='"+style+"'>"+value+"</div>";
  obj2.innerHTML = val;
  if (val.indexOf("<img")!=-1) obj2.style.height = obj2.offsetHeight+"px";
  if (currRow == -1 || currCol == -1) highlightCellHeader(currRow,currCol);
  //previewValue();
}
function setBoldCellMatrix(){
  $(".cell_highlight_over").each(function(index,elem){
      var matrix = elem.id.toString().split("_");
      if (cells[matrix[0]][matrix[1]])
        setFormatCell(matrix[0],matrix[1],"font-weight:bold");
  });
}
function setItalicCellMatrix(){
  $(".cell_highlight_over").each(function(index,elem){
      var matrix = elem.id.toString().split("_");
      if (cells[matrix[0]][matrix[1]])
        setFormatCell(matrix[0],matrix[1],"font-style:italic");
  });
}
function setUnderlineCellMatrix(){
  $(".cell_highlight_over").each(function(index,elem){
      var matrix = elem.id.toString().split("_");
      if (cells[matrix[0]][matrix[1]])
        setFormatCell(matrix[0],matrix[1],"text-decoration:underline");
  });
}
function setFormatCell(row,col,format_cell) {
  var obj2 = resolveCell(row,col);
  var pos_bold = -1;
  var value = previewCell(cells[row][col][0],0);
  var style = cells[row][col][1];
  value = formatValue(value,style);
  style = htmlEscape(formatStyle(style,value),false);
  //alert(style);
  var arrstyle = style.split(";");
  for (var i=0; i<arrstyle.length; i++){
    if (arrstyle[i].replace(" ","") == format_cell){
      pos_bold = i;
      break;
    }
  }
